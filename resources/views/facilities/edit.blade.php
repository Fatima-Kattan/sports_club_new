<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="image/svg+xml"
        href="data:image/svg+xml;base64,">
    <title>edit facility</title>
    <link rel="stylesheet" href="{{ asset('css/facility.css') }}">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Cairo:wght@300;400;500;600;700&display=swap" rel="stylesheet">
</head>

<body>
    <div class="container" style="max-width: 800px;">
        <header class="main-header">
            <h1><i class="fas fa-edit"></i> Edit Facility</h1>
            <p class="subtitle">Update the facility information</p>
        </header>

        {{-- resources/views/facilities/edit.blade.php --}}
        <div class="form-container">
            {{--  The route points to the update method (usually facilities.update)  --}}
            <form action="{{ route('facilities.update', $facility->id) }}" method="POST" class="facility-form">
                @csrf
                {{--  Laravel expects a PUT/PATCH for updates – we spoof it with @method  --}}
                @method('PUT')

                <div class="form-group">
                    <label for="room_name">
                        <i class="fas fa-door-open"></i> Room name
                    </label>
                    {{--  Fill the input with the current value; fallback to old() after validation error  --}}
                    <input type="text" id="room_name" name="room_name" required
                        placeholder="Enter the room name (e.g., Weight Room, Swimming Pool)"
                        value="{{ old('room_name', $facility->room_name) }}">
                </div>

                <div class="form-group">
                    <label for="floor">
                        <i class="fas fa-stairs"></i> Floor
                    </label>
                    <input type="text" id="floor" name="floor" required
                        placeholder="Enter the floor number (e.g., first, second)"
                        value="{{ old('floor', $facility->floor) }}">
                </div>

                <div class="form-group">
                    <label for="room_capacity">
                        <i class="fas fa-users"></i> Capacity
                    </label>
                    <input type="number" id="room_capacity" name="room_capacity" required min="1"
                        placeholder="Enter the maximum capacity for people"
                        value="{{ old('room_capacity', $facility->room_capacity) }}">
                </div>

                <div class="form-actions">
                    <button type="submit" class="btn-submit">
                        <i class="fas fa-save"></i> Update facility
                    </button>
                    <a href="{{ route('facilities.index') }}" class="btn-cancel">
                        <i class="fas fa-times"></i> Cancel
                    </a>
                </div>
            </form>
        </div>
    </div>
</body>

</html>
