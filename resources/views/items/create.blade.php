...
<body>
    ...
    <nav class="item-navbar">
        <div class="container">
            ...
            <div class="nav-links">
                <a href="{{ route('categories.show', $category->id) }}">Back to {{ $category->name }}</a>
    ...
    <div class="container">
        ...
        <div class="page-header">
            <h1>Add New Item</h1>
            <p>Add a new equipment to <span class="category-name">{{ $category->name }}</span></p>
        </div>

        <!-- Form Card -->
        <div class="form-card">
            <form action="{{ route('items.store', $category->id) }}" method="POST" id="itemForm"
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" required
                        placeholder="e.g., Treadmill, Dumbbell Set, Yoga Mat" value="{{ old('name') }}">
                    @error('name')
                ...
                <div class="form-group">
                    <label for="image">
                        Item Image <span class="required">*</span>
                        <span class="hint">(Max: 2MB, Formats: JPEG, PNG, JPG, GIF, SVG)</span>
                    </label>
                    ...
                    <div class="image-upload-container">
                        ...
                        <div class="upload-controls">
                            ...
                            <button type="button" class="clear-btn" id="clearImage" style="display: none;">
                                <i class="fas fa-times"></i> Remove
                            </button>
                        </div>

                        <div class="upload-info">
                            <p class="info-text">Click or drag & drop to upload image</p>
                ...
                <div class="form-group">
                    ...
                    <div class="status-options">
                        @foreach (['available', 'Not available', 'out of service', 'under maintenance'] as $status)
                            <label class="status-option">
                                <input type="radio" name="status" value="{{ $status }}"
                                    {{ old('status', 'available') == $status ? 'checked' : '' }}>
                                <span class="status-label {{ $status }}">
                                    @switch($status)
                                        ...
                                        @case('out of service')
                                            <i class="fas fa-wrench"></i> Out of Service
                                        @break
                ...
                <div class="form-actions">
                    ...
                    <a href="{{ route('categories.show', $category->id) }}" class="btn-cancel">
