...
<body>
    <div class="form-container">
        ...
        <!-- نموذج التعديل -->
        <form action="{{ route('activities.update', $activity) }}" method="POST" enctype="multipart/form-data" id="activityForm" class="form-card">
            @csrf
            @method('PUT')
            ...
            <div class="form-group full-width">
                ...
                <div class="form-help">
                    Maximum 500 characters. Currently: <span id="charCount">{{ strlen(old('description', $activity->description)) }}</span>/500
                </div>
            </div>
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <select id="level" name="level" class="form-select" required>
                        ...
                        <option value="Beginner" {{ old('level', $activity->level) == 'Beginner' ? 'selected' : '' }}>Beginner</option>
                        <option value="Intermediate" {{ old('level', $activity->level) == 'Intermediate' ? 'selected' : '' }}>Intermediate</option>
                        <option value="Advanced" {{ old('level', $activity->level) == 'Advanced' ? 'selected' : '' }}>Advanced</option>
                        <option value="All Levels" {{ old('level', $activity->level) == 'All Levels' ? 'selected' : '' }}>All Levels</option>
                    </select>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const removeCurrentImageCheckbox = document.getElementById('remove_current_image');
    ...
    // تحذير عند محاولة إزالة الصورة الحالية
    if (removeCurrentImageCheckbox) {
        removeCurrentImageCheckbox.addEventListener('change', function() {
            if (this.checked) {
        ...
        if (resetBtn) {
            resetBtn.addEventListener('click', function(e) {
                if (formChanged) {
                    ...
                    if (previewImg) previewImg.src = '';
                    if (imageInput) imageInput.value = '';
                    // إلغاء اختيار حذف الصورة الحالية
                    if (removeCurrentImageCheckbox) removeCurrentImageCheckbox.checked = false;
